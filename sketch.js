...
function setup() {
    ...
    world = engine.world;
    fort = Bodies.rectangle(200, 350, 160, 310, { isStatic: true });
    World.add(world,fort);
   tank=new Tank(500,500,250,250)
     mg=new Maingun(600,480,55,55,angle)
  ...
  function draw() 
{
  ...
  imageMode(CENTER);
  image(fortImage, 50,150, 160, 310);
  pop();
  ...
  function showenemies() {
    if(enemies.length>0) {
      ...
      for (var i = 0; i < enemies.length; i++) {
        if (enemies[i]) {
          enemies[i].display();
          Matter.Body.setVelocity(enemies[i].body,{x:-0.9 ,y:-0.4});
          var collision=Matter.SAT.collides(fort,enemies[i].body)
          if(collision.collided){
gameOver()
